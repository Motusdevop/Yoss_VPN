...
"""
        ...
        await callback.message.answer(
            text=text, reply_markup=MyVPNKeyboard.markup, parse_mode=ParseMode.HTML
        )
    ...
    if callback.data != "cancel":
        ...
        if callback.data == "recreate":
            ...
            if datetime.now() > subscription.expires_on:
                await state.clear()
                await callback.message.answer(
                    "Ваша подписка больше не действует, сначала продлите её"
                )
                ...
                if api.ping(ip_address):
                    ...
                    result = api.create_config(
                        config_name=config.name, ip_address=ip_address
                    )
                    ...
                    await callback.message.answer(
                        f"```{file}```", parse_mode=ParseMode.MARKDOWN
                    )
