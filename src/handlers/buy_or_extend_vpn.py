...
from keyboards import ServerKeyboard, TariffKeyboard, CheckPay
from models import Config, Subscription, Transaction
from repository import UserRepository, SubscriptionRepository, ConfigRepository, ServerRepository, TariffRepository, \
...
async def buy_or_extend(callback: CallbackQuery, state: FSMContext):
    if callback.data == 'buy':
        ...
        for subscription in subscriptions_list:
            ...
            kb.append([InlineKeyboardButton(text=text, callback_data=str(subscription.id))])
        ...
        await callback.message.answer('Выберите конфиг для продления', reply_markup=markup)
...
async def select_server(callback: CallbackQuery, state: FSMContext):
    if callback.data != 'no_servers':
        ...
        tariff_keyboard = TariffKeyboard(UserRepository.get_from_chat_id(callback.from_user.id))
...
async def select_tariff(callback: CallbackQuery, state: FSMContext):
    if callback.data != 'cancel':
        if callback.data == 'free_trial':
            ...
            config = vpn.create_config(username=user.username, user_id=user.id, server_id=server_id)
            subscription_id = vpn.create_subscription(user_id=user.id, config_id=config.id, days=2)
...
Благодарим за доверие!
            ...
            if callback.data == 'one_month':
                ...
                await callback.message.answer(text, reply_markup=CheckPay.markup, parse_mode=ParseMode.MARKDOWN)
...
async def check_pay(callback: CallbackQuery, state: FSMContext):
    if callback.data == 'check':
        ...
        if 'subscription_id' in data.keys():
            ...
            transaction = Transaction(user_id=user.id, server_id=server_id,
                                      tariff_id=tariff.id, subscription_id=subscription_id)
        else:
            transaction = Transaction(user_id=user.id, server_id=data['server_id'],
                                      tariff_id=tariff.id)
