...
from forms.admin import CheckPay, SubscriptionManager, UserManager
from handlers.base import check_register
from keyboards import SubscriptionKeyboard, UserManagementKeyboard
from models import Config, Server, Subscription, Transaction
...
from tools import api, bot_mode, vpn
    ...
    if await check_register(message, state):
        ...
        if user.role == "admin":
            ...
            try:
                ...
                await message.answer(
                    f"id={user.id}\nusername={user.username}\nrole={user.role}",
                    reply_markup=UserManagementKeyboard.markup,
                )
    ...
    if callback.data == "delete":
        ...
        for subscription in subscription_list:
            ...
            ServerRepository.update(
                server.id, count_of_configs=server.count_of_configs - 1
            )
        ...
        if user.role == "admin":
            ...
            if len(transaction_list) == 0:
                ...
                for transaction in transaction_list:
                    ...
                    text += (
                        f"{user.username}, +{user.phone} перевёл {tariff.price} на СБП"
                    )
                    ...
                    kb = [
                        [
                            ...
                            InlineKeyboardButton(
                                text="Отмена", callback_data=f"cancel {transaction.id}"
                            ),
                        ]
                    ]
        ...
        if callback.data.split()[0] == "confirm":
            if transaction.subscription_id is None:
                ...
                config = vpn.create_config(
                    username=user.username, user_id=user.id, server_id=server.id
                )
                days = 30 if tariff.id == 1 else 90
                print(days)
                subscription_id = vpn.create_subscription(
                    user_id=user.id, config_id=config.id, days=days
                )
...
"""
                ...
                if subscription.expires_on > datetime.now():
                    SubscriptionRepository.set_expired_on(
                        subscription.id, subscription.expires_on + add_time
                    )
                else:
                    SubscriptionRepository.set_expired_on(
                        subscription.id, datetime.now() + add_time
                    )
                    config = ConfigRepository.get(subscription.config_id)
                    server = ServerRepository.get(config.server_id)
...
"""
            ...
            await callback.bot.send_message(
                user.chat_id,
                "Ваш платёж недействителен, для помощи напишите @Kapchonka77",
            )
